...
double distance_step, angular_step;
...
void LoadConfigFile(const std::string &file)
{
    ...
    distance_step = fSettings["distance_step"];
    angular_step = fSettings["angular_step"];
...
int main(int argc, char **argv)
{
    ...
    // ROS节点初始化
    ros::init(argc, argv, "dashgo_mapping");
    // 创建节点句柄
//    ros::NodeHandle node;
...
//    ros::Rate rate(10);
    ...
    while(1)
    {
        ...
        mpDrawer->SetCurPose(cur_pose);
        cout << cur_pose.x() << ", " << cur_pose.y() << ", " << cur_pose.z() << ", " << total_dist << ", " << total_angle << endl;
        if(d_t > distance_step || d_theta > angular_step)// keyframe selection
        {
            total_angle += d_theta;
            total_dist += d_t;
