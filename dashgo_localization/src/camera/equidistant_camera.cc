...
bool
EquidistantCamera::Parameters::readFromYamlFile(const std::string& filename, uchar lr)
{
    ...
    if(lr == 'l')
    {
        std::cout << "read left camera parameters:" << std::endl;
        fs["intrinsics_l"] >> intrinsic;
        fs["distortion_coefficients_l"] >> dist_coef;
    }
    else if(lr == 'r')
    {
        std::cout << "read right camera parameters:" << std::endl;
        fs["intrinsics_r"] >> intrinsic;
        fs["distortion_coefficients_r"] >> dist_coef;
    }
