...
// 将当前Node拆分为4个Node，并把keypoints放进去
...
static bool compareNodes(pair<int, _ExtractorNode *> &e1, pair<int, _ExtractorNode *> &e2)
{
      ...
      initial_threshold_fast(_initial_threshold_fast), min_threshold_fast(_min_threshold_fast)
{
    ...
    // TODO  ndesiredFeaturesPerScale表示第一层应该分配多少点（这里是按边长均分，不是应该按面积均分吗？）
    int sumFeatures = 0;
  ...
  //  pre-compute the end of a row in a circular patch
    ...
    umax.resize(half_patch_size + 1);// orb3源码
//    umax.resize(half_patch_size + 2);//TODO Opencv源码
...
void ORB_Extractor::Detect(cv::InputArray _image, vector<cv::KeyPoint> &_keypoints, cv::OutputArray _descriptors)
{
    assert(!_image.empty());
...
void ORB_Extractor::ComputeKeyPointsOctTree(vector<vector<cv::KeyPoint>> &allKeypoints)
{
    ...
    for(int level = 0; level < n_levels; ++level)
    {
        ...
        // TODO 这里keypoints是在加了edge_threshold图像中的坐标
...
vector<cv::KeyPoint> ORB_Extractor::DistributeOctTree(const vector<cv::KeyPoint> &vToDistributeKeys,
                                                      ...
                                                      const int &maxY, const int &N)
{
    ...
    list<_ExtractorNode> lNodes;

    vector<_ExtractorNode *> vpIniNodes;
    ...
    list<_ExtractorNode>::iterator lit = lNodes.begin();
    ...
    vector<pair<int, _ExtractorNode *>> vSizeAndPointerToNode;
    ...
    while(!bFinish)
    {
        ...
        else if(((int)lNodes.size() + nToExpand * 3) > N)
        {
            while(!bFinish)
            {
                ...
                vector<pair<int, _ExtractorNode *>> vPrevSizeAndPointerToNode = vSizeAndPointerToNode;
                vSizeAndPointerToNode.clear();
    ...
    for(list<_ExtractorNode>::iterator lit = lNodes.begin(); lit != lNodes.end(); lit++)
    {
                             ...
                             int edgeThreshold, int patchSize, int fastThreshold)
{
    ...
    // ndesiredFeaturesPerScale表示第一层应该分配多少点（这里是按边长均分，不是应该按面积均分吗？）
    ...
    nfeaturesPerLevel[nlevels-1] = std::max(nfeatures - sumFeatures, 0);// 剩余的都给最后一层
...
void detectAndCompute( InputArray _image, std::vector<KeyPoint>& keypoints, OutputArray _descriptors)
{
    ...
    Point level_ofs(0,0);
    Size bufSize((image.cols + border*2 + 15) & -16, 0);// 816*0
    // bufSize.width = 816,    即等于image.cols + border*2 + 15 - 16 + 1;
    ...
    for(int level = 0; level < nLevels; ++level)
    {
        ...
        Mat extImg = imagePyramid(wholeLinfo);// 含边界border
        Mat currImg = extImg(Rect(border, border, sz.width, sz.height));// 不含边界
